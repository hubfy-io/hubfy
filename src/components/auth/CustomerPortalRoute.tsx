...
export function CustomerPortalRoute({
  ...
  const {
    ...
    tenant?.id ?? undefined,
    Boolean(user && accessRole === "customer" && !allowOnboarding),
  );

  /* ── Loading ── */
  if (authLoading || portalLoading || onboardingLoading) {
    return (
  ...
  if (accessRole === "customer" && requiredOnboarding && !allowOnboarding) {
    return (
      <Navigate
        to={`/${slug}/onboarding`}
        state={{ from: location.pathname }}
  ...
  return (
    <PortalProvider tenant={tenant} customer={customer} accessRole={accessRole}>
      {accessRole === "customer" && !allowOnboarding && <SetPasswordDialog />}
      {children}
    </PortalProvider>
  );
}
