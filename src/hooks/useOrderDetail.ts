...
interface OrderDetailRow {
  ...
  checkouts: { smart_id: string } | null;
}
  ...
  const { data: order, isPending, error } = useQuery({
    ...
    queryFn: async (): Promise<OrderDetail | null> => {
      ...
      const { data, error } = await supabase
        .from("orders")
        .select(
          "*, customers!orders_customer_id_fkey(public_id, name, email, phone, city, region, country, document, document_type), products!orders_product_id_fkey(public_id, name, benefit, cover_url), checkouts(smart_id)"
        )
      ...
      return {
        ...
        checkout_slug: o.checkouts?.smart_id ?? null,
      };
    },
