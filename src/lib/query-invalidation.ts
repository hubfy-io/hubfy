...
/**
 * Invalidate checkout queries (admin + any portal view showing checkout covers).
 */
export function invalidateCheckouts(qc: QueryClient, tenantId: string | null) {
  qc.invalidateQueries({ queryKey: ["checkouts", tenantId] });
  // Products list also shows linked checkouts
  invalidatePortalProducts(qc);
}
