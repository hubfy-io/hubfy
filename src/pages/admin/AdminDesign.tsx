...
import DesignCheckoutTab from "@/components/admin/design/DesignCheckoutTab";
import type { DesignCheckoutFormData } from "@/components/admin/design/DesignCheckoutTab";
...
const DESIGN_TAB_LABEL_KEYS: Record<Tab, string> = {
  ...
  checkout: "designPage.tabs.checkout",
...
function getDesignPreviewUrl(activeTab: Tab, slug?: string | null) {
  ...
  if (activeTab === "video-player") {
    return "play.hubfy.io/video_73fs6454dvr592134";
  }
  ...
  return "seusite.com/checkout/preview";
}
...
export default function AdminDesign() {
  ...
  // Form state — Checkout
  const [checkoutData, setCheckoutData] = useState<DesignCheckoutFormData>({
    checkout_use_brand_colors: true,
    checkout_bg_color: "#F9F9F9",
  });
  ...
  useEffect(() => {
    if (tenant) {
      ...
      setFormData({
        ...
        button_style: tenant.checkout_button_style || "pill",
      });
      setCheckoutData({
        checkout_use_brand_colors: tenant.checkout_use_brand_colors ?? true,
        checkout_bg_color: tenant.checkout_bg_color || "#F9F9F9",
      });
      setLoginPageData({
  ...
  const hasChanges = useMemo(() => {
    ...
    const checkoutChanged =
      checkoutData.checkout_use_brand_colors !== initialCheckoutData.checkout_use_brand_colors ||
      checkoutData.checkout_bg_color !== initialCheckoutData.checkout_bg_color;
    ...
    return generalChanged || checkoutChanged || loginPageChanged || portalChanged || videoChanged || marketingChanged;
  }, [
    formData,
    initialData,
    checkoutData,
  ...
  const handleCheckoutChange = (updates: Partial<DesignCheckoutFormData>) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateTenant({
        ...
        checkout_button_style: formData.button_style,
        checkout_button_color: null,
        checkout_use_brand_colors: checkoutData.checkout_use_brand_colors,
        checkout_bg_color: checkoutData.checkout_use_brand_colors ? null : checkoutData.checkout_bg_color,
                  ...
                  <div className="h-full overflow-y-auto">
                    {activeTab === "video-player" ? (
                      ...
                      <DesignPreview
                        ...
                        previewMode={previewMode}
                        checkoutBgColor={
                          checkoutData.checkout_use_brand_colors
                            ? undefined
                            : checkoutData.checkout_bg_color
                        }
                        checkoutButtonStyle={formData.button_style}
                      />
                    )}
                  </div>
                </BrowserChrome>
