...
import { ActiveCampaignDialog } from "@/components/admin/integrations/ActiveCampaignDialog";
...
export default function AdminIntegrations() {
  ...
  const { isConnected: kiwifyConnected } = useGatewayIntegration("kiwify");
  const { isConnected: kirvanoConnected } = useGatewayIntegration("kirvano");
  const { isConnected: lastlinkConnected } = useGatewayIntegration("lastlink");
  const { isConnected: greennConnected } = useGatewayIntegration("greenn");
  ...
  const { isConnected: activecampaignConnected } = useSimpleIntegration("activecampaign", {
  ...
  function getIsConnected(key: ProviderKey): boolean {
    ...
    if (key === "kiwify") return kiwifyConnected;
    if (key === "kirvano") return kirvanoConnected;
    if (key === "lastlink") return lastlinkConnected;
    if (key === "greenn") return greennConnected;
    if (key === "vimeo") return vimeoConnected;
    if (key === "openai") return openaiConnected;
    if (key === "anthropic") return anthropicConnected;
    if (key === "activecampaign") return activecampaignConnected;
  ...
  const DIALOG_PROVIDERS: Record<string, () => void> = {
    activecampaign: () => setAcDialogOpen(true),
  };
  ...
  return (
    <>
      ...
      {/* Dialog — Active Campaign */}
      <ActiveCampaignDialog
        open={acDialogOpen}
