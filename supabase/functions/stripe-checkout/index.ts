...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 2. Parse body
    const { tenant_id, price_id, success_path, cancel_path } = await req.json();
    if (!tenant_id) {
    ...
    if (!price_id) {
      return new Response(JSON.stringify({ error: "price_id is required", code: "missing_required_field" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // 4. Stripe: busca customer por email, se não existe cria novo
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}${success_path || "/admin?checkout=success"}`,
      cancel_url: `${baseUrl}${cancel_path || "/admin?checkout=canceled"}`,
      metadata: { tenant_id, user_id: userId },
    });
