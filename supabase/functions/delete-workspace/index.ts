...
async function cancelStripeSubscriptionBestEffort(
  ...
  if (!stripeSubscriptionId || status !== "active") return;

  const stripeSecretKey = Deno.env.get("STRIPE_SECRET_KEY");
  if (!stripeSecretKey) {
    console.warn(
      "STRIPE_SECRET_KEY not set, skipping Stripe subscription cancellation",
    );
    return;
  }
  ...
  try {
    ...
    if (!stripeRes.ok && stripeRes.status !== 404) {
      ...
      console.warn(
        `Stripe cancel failed (${stripeRes.status}) for ${stripeSubscriptionId}: ${errorText}`,
      );
    }
  } catch (error) {
