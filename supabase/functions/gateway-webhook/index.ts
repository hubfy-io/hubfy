...
Deno.serve(async (req: Request) => {
  ...
  if (!adapter.validateAuth(req, rawBody, body, credentials)) {
    // TEMP (Greenn descoberta de header): logar apenas os NOMES dos headers
    // recebidos quando falha auth em greenn, pra descobrir qual header a Greenn
    // está usando. Remover após confirmar o header correto.
    if (provider === "greenn") {
      const headerNames = Array.from(req.headers.keys()).filter(
        (k) => !["content-type", "content-length", "host", "accept", "accept-encoding", "user-agent", "connection"].includes(k),
      );
      console.log("[gateway-webhook] greenn auth_failed — header names received:", headerNames);
    }
