...
-- These are safe to store — anon key is public (already in frontend)
-- ============================================================
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM vault.decrypted_secrets WHERE name = 'supabase_url') THEN
    PERFORM vault.create_secret(
      'https://xrspiwankdnkqaezfhjt.supabase.co',
      'supabase_url',
  ...
  IF NOT EXISTS (SELECT 1 FROM vault.decrypted_secrets WHERE name = 'supabase_anon_key') THEN
    PERFORM vault.create_secret(
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inhyc3Bpd2Fua2Rua3FhZXpmaGp0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAzMTg4MDAsImV4cCI6MjA4NTg5NDgwMH0.jqNC2ApZsRSZ711mj9AMeE42Fd4xjCyimN0qwPOIa8k',
      'supabase_anon_key',
      'Supabase anon key for pg_net triggers'
    );
  END IF;
END;
$$;
